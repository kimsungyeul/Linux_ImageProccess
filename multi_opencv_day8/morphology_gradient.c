    ...
    for(y = 1; y < bmpInfoHeader.biHeight+1; y++) { 
        for(x = 1*elemSize; x < padSize; x+=elemSize) {
            for(z = 0; z < elemSize; z++) {
                ...
                qsort(data,sizeof(data)/sizeof(int),sizeof(int),compare);
                #if 0
					outimg[(x-elemSize)+(y-1)*size+z] = LIMIT_UBYTE(outimg[(x-elemSize)+(y-1)*size+z] -= LIMIT_UBYTE(data[0]));
                #else
					outimg[(x-elemSize)+(y-1)*size+z] -= LIMIT_UBYTE(data[0]);
					outimg[(x-elemSize)+(y-1)*size+z] = LIMIT_UBYTE(outimg[(x-elemSize)+(y-1)*size+z]);
                #endif
            }
        }
    }
